...
/**
 * @author xiaocoder
 * @date 2015-3-2 下午5:25:34
 */
public class XCPop {
	private static XCPop instance = new XCPop();
	public ViewGroup popup_layout;
	public PopupWindow popup_window;

	private XCPop() {
	};

	public static XCPop getXCPopInstance() {
		return instance;
	}

	/*
	 * pop_layout_id : pop里面的布局 width: pop的宽 height: pop的高 anim_style: 动画效果 ,
	 * 如果anim_style<=0 , 则表示没有动画
	 */
	public PopupWindow getPopupWindow(Context context, int pop_layout_id, int width, int height, int anim_style) {
		LayoutInflater inflater = LayoutInflater.from(context);
		popup_layout = (ViewGroup) inflater.inflate(pop_layout_id, null);
		popup_window = new PopupWindow(popup_layout, width, height);
		popup_window.setBackgroundDrawable(new ColorDrawable());// 一个空图片
		popup_window.setFocusable(true);
		popup_window.setOutsideTouchable(true);
		if (anim_style > 0) {
			popup_window.setAnimationStyle(anim_style);
		}
		return popup_window;
	}

	public void showPopupWindow(View view, int xoff, int yoff) {
		popup_window.showAsDropDown(view, xoff, yoff);
	}
}
