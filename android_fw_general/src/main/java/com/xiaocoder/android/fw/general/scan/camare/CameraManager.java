package com.xiaocoder.android.fw.general.scan.camare;
...
public final class CameraManager {
	...
	private static final int MAX_FRAME_WIDTH = 550; // 这里可以修改扫描框的宽
	// private static final int MAX_FRAME_HEIGHT = 360;
	private static final int MAX_FRAME_HEIGHT = 500; // 这里可以修改扫描框的高
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			int topOffset = (screenResolution.y - height) / 3; // 这句可以修改取景框距离顶部的位置
	 ...
	 * 
	 * @param points
	 *            The points returned by the Reader subclass through Result.getResultPoints().
	 * @return An array of Points scaled to the size of the framing rect and offset appropriately so they can be drawn in screen coordinates.
	 */
	/*
	 * public Point[] convertResultPoints(ResultPoint[] points) { Rect frame = getFramingRectInPreview(); int count = points.length; Point[] output = new Point[count]; for (int x = 0; x < count; x++)
	 * { output[x] = new Point(); output[x].x = frame.left + (int) (points[x].getX() + 0.5f); output[x].y = frame.top + (int) (points[x].getY() + 0.5f); } return output; }
	 */
