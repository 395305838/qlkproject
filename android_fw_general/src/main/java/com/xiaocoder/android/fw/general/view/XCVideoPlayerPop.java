 ...
 * @author honor
 */
public class XCVideoPlayerPop implements OnErrorListener, OnCompletionListener, OnBufferingUpdateListener, OnVideoSizeChangedListener, OnPreparedListener, Serializable {
    ...
    public XCVideoPlayerPop(Context context, Uri uri, SurfaceView surface_view) {
    ...
    private void init() {
        ...
        surface_view_height = screenHeight; // 初始化时默认是全屏播放
        surface_view_width = screenWidth;
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        XCApplication.printi("up" + down_time);
                        break;
                    case MotionEvent.ACTION_MOVE: //  -->这里是可以双向滑动的
                        ...
                        XCApplication.printi("move_y1 is " + y1); // 每一个滑动过程中
                        XCApplication.printi("move_y2 is " + y2); // y2是变化的
    ...
    private void setVolumn(int direction) {
        if (audioManager != null) {
            ...
            XCApplication.printi("current--" + currentVolumn);
            XCApplication.printi("maxVolumn--" + maxVolumn);
...
//			((Activity)context).setVolumeControlStream(AudioManager.STREAM_MUSIC);
            ...
            player.prepareAsync();
            XCApplication.printi("player has been created");
    ...
    private void setSurfaceViewScaleParam(int width, int height) {
        ...
        surface_view_height = height;
        surface_view_width = width;
    ...
    Runnable enterBlack = new Runnable() {
        ...
        public void run() {
            ...
            if (surface_view_height == screenHeight) {
                setSurfaceViewScale(SCALE_TO_FIFTY);
            } else if (surface_view_height == (int) screenHeight / 1.5) {
                setSurfaceViewScale(REAL_OF_VIDEO);
            } else {
                setSurfaceViewScale(FULL_SCREEN);
            }
        }
    };
    ...
    private void showPop() {
        ...
        XCApplication.printi( "showController");
    }
    ...
    private void dismissPop() {
        ...
        XCApplication.printi("dismissController");
    }
    ...
    private int getCurrentPosition() {
        if (player != null && isPrepared) {
            XCApplication.printi("getCurrentPosition");
    ...
    public void onCompletion(MediaPlayer mp) {
        XCApplication.printi("onCompletion" + "-isLoop" + isLoop);
    ...
    public void onBufferingUpdate(MediaPlayer mp, int percent) {
        XCApplication.printi("onBufferingUpdate---" + percent);
//		isPrepared = false;
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        XCApplication.printi("onVideoSizeChangedListener" + "--width" + width + "/height" + height);
        videoHeight = height;
        videoWidth = width;
    }
    ...
    public void onPrepared(MediaPlayer mp) {
        XCApplication.printi("OnPreparedListener" + "--the width of mediaplayer is " + player.getVideoWidth() + "--the height of mediaplayer is" + player.getVideoHeight());
        // 缓冲好了后,设置seekbar的最大值
        ...
        XCApplication.printi("seekToWhenPreparedFinished" + seekToWhenPreparedFinished);
    }
