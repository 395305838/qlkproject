...
public class XCVideoPlayerPop implements OnErrorListener, OnCompletionListener, OnBufferingUpdateListener, OnVideoSizeChangedListener, OnPreparedListener, Serializable {
    ...
    private void init() {
        ...
        volumn_or_brightness_changed.setBackgroundResource(R.drawable.xc_dd_volumn_or_brightness_background_shape);
        ...
        volumn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (!isVideoSlient) {
                    ...
                    isVideoSlient = true;
                    volumn.setImageResource(R.drawable.xc_dd_soundmute);
                    return;
                }
                audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, currentVolumn, 0);
                isVideoSlient = false;
                volumn.setImageResource(R.drawable.xc_dd_soundcontrol);
            }
        });
    ...
    private void setBrightness(int direction) {
        ...
        volumn_or_brightness_imageview.setImageResource(R.drawable.xc_d_player_icon_bright);
    ...
    private void setVolumn(int direction) {
        if (audioManager != null) {
            if (isVideoSlient) {// 是否静音
                // 设置当前音量
                volumn_or_brightness_imageview.setImageResource(R.drawable.xc_d_soundmute_normal);
...
//		isPrepared = false;
    ...
    private void start() {
        if (player != null && isPrepared) {
            ...
            player.start();
            pause_or_start.setImageResource(R.drawable.xc_dd_pause);
